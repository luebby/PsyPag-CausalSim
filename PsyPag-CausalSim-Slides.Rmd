...
Or as [Thomas Lumley put it:](https://notstatschat.rbind.io/2021/05/02/generalisability-prediction-and-causation/)
...
- Data is not just there - it has a generating process and we should care about this. Simulations are very good way to think about this.
...
What do you think ist the magnitude of the gender pay gap for women in this company?
...
What do you think ist the magnitude of the healthy pay gap in this company?
...
$\color{green}{X}$: .green[learning], $\color{violet}{Z}$: .violet[knowing], $\color{blue}{Y}$: .blue[understanding]. *Structual Causal Model*:
...
- The value of $\color{blue}{Y}$ depends on $\color{green}{X}$ - but the value of $\color{green}{X}$ **not** on $\color{blue}{Y}$. 
...
.pull-left[
Within a randomized controlled trial observation $i$ is randomly assigend to $\color{green}{x}_i$. The connections to the *parents* of $\color{green}{X}$ is cut.
]
...
- learnR Tutorial with real data: [https://fomshinyapps.shinyapps.io/CausalInference/](https://fomshinyapps.shinyapps.io/CausalInference/)
